...
import static org.solidcoding.validation.predicates.NumberPredicate.beANumber;
...
class NumberPredicateTests {
    ...
    void itShould_integerMatchingRule_shouldReturnTrue() {
        var value = 2;
        var rule0 = DefineThat.itShould(beANumber(2)).otherWiseReport("failure");
        var rule1 = DefineThat.itShould(beANumber().between(1).and(5)).otherWiseReport("failure");
        var rule2 = DefineThat.itShould(beANumberOfLength(1)).otherWiseReport("failure");
        var rule3 = DefineThat.itShould(beANumber(that -> that.equals(2))).otherWiseReport("failure");
        var rule4 = DefineThat.itShould(beANumber().containing(2)).otherWiseReport("failure");
    ...
    void itShould_integerNotMatchingRule_shouldReturnFalse() {
        var value = 2;
        var rule0 = DefineThat.itShould(beANumber(1)).otherWiseReport("failure");
        var rule1 = DefineThat.itShould(beANumber().between(5).and(50)).otherWiseReport("failure");
        var rule2 = DefineThat.itShould(beANumberOfLength(10)).otherWiseReport("failure");
        var rule3 = DefineThat.itShould(beANumber(that -> that.equals(5435))).otherWiseReport("failure");
    ...
    void itShould_extendedTests() {
        var value = 123456789;
        var rule1 = DefineThat.itShould(beANumber().containing(1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 23, 789)).otherWiseReport("failure");
