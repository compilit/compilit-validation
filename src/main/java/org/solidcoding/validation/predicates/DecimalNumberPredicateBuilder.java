...
public final class DecimalNumberPredicateBuilder extends ObjectPredicateBuilder<Double> {
  ...
  private DecimalNumberPredicateBuilder(final List<Predicate<Double>> predicates) {
    super(predicates);
  }
   ...
   */
  public static PredicateConnector<Double, Predicate<Double>> isBetween(final double first) {
    return new DecimalNumberConstraintPredicateConnector(first, new ObjectPredicateBuilder<>());
  }
   ...
   * @return Predicate to continue adding rules.
   */
  public static GenericPredicate<Double> contains(final Integer value, final Integer... values) {
    final var rules = new ArrayList<Predicate<Double>>();
   ...
   * @return Predicate to continue adding rules.
   */
  public static GenericPredicate<Double> doesNotContain(final Integer value, final Integer... values) {
    final var rules = new ArrayList<Predicate<Double>>();
    rules.add(x -> !x.toString().contains(value.toString()));
    for (final var c : values) {
      rules.add(x -> !x.toString().contains(c.toString()));
